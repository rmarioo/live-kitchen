...
ENTRYPOINT ["java","-Dspring.datasource.url=${DATASOURCE_URL}","-Dchefserver.host=${CHEF_SERVER_URL}","-XX:SharedArchiveFile=application.jsa", "-jar", "myapp/myapp.jar"]

COPY --from=reduced_optimizer /app ./

RUN java  -XX:ArchiveClassesAtExit=./application.jsa -Dspring.datasource.url=${DATASOURCE_URL} -Dchefserver.host=${CHEF_SERVER_URL} -Dspring.context.exit=onRefresh -jar myapp/myapp.jar
